...
namespace MFST
{
	...
	bool Mfst::start()
	{
		...
		case LENTA_END:
		{
			MFST_TRACE4("------>LENTA_END")
				cout << "------------------------------------------------------------------------------------------------\n";
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: всего строк %d, синтаксический анализ выполнен без ошибок", 0, lex.table[lex.size - 1].numberOfString);
			cout << setw(4) << left << 0 << "всего строк " << lex.table[lex.size - 1].numberOfString << ", синтаксический анализ выполнен без ошибок\n";
			rc = true;
			break;
		}
